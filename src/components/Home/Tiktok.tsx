...
export default function Posts({ posts }: { posts: Post[] }) {
  return (
    <div className="w-full">
      <Link href="/posts">
        <h2 className="text-4xl font-black hover:text-gray-500">Tiktok</h2>
      </Link>
      ...
      <div className="my-10">
        ...
        >
          {posts.map((post) => (
            <SwiperSlide
              key={post.createdAt}
              className="overflow-hidden relative group h-[480px] w-[270px] border-4 border-[#d6001d]"
            >
              <Link
                href={post.url}
                target="_blank"
                className="shadow-xl h-[480px] w-[270px]"
              >
                <Image
                  src={post.imageUrl}
                  width={400}
                  height={600}
                  className="w-full h-[480px] object-center object-fill "
                  alt={"posts"}
                  priority
                />
